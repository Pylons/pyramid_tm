...
are tested:
  ...
  3) If the ``pyramid_tm.commit_veto`` configuration setting was used, did
...
(``True`` or ``False``), then pass a ``pyramid_tm.commit_veto`` key/value
...
.. code-block:: python
   ...
   def app(global_conf, settings):
       settings['pyramid_tm.commit_veto'] = 'my.package.commit_veto'
       config = Configurator(settings=settings)
       config.include('pyramid_tm')
